...
local fadedRects = {} -- precompute fade
for i=0,1,0.01 do
...
function SceneManager:switchScene(scene, ...)
    ...
    self.newScene = scene
    self.sceneArgs = ... -- scene data which can be passed from anywhere
...
function SceneManager:startTransition()
    --local transitionTime = self:wipeTransition(0,400)
    local transitionTime = self:fadeTransition(0,1)

    transitionTime.timerEndedCallback = function ()
        self:loadNewScene()
...
function SceneManager:createTransitionSprite()
    local filledRect = gfx.image.new(400,240, gfx.kColorBlack)
    --local filledRect = gfx.image.new("images/coin")
...
function SceneManager:cleanScene()
